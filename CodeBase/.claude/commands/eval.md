...
Run `backend/evals/run_eval.py`. Label: $ARGUMENTS
...
1. Run the harness. Quote its actual output. `run_eval.py` exits non-zero on gate failure; report
   the exit code.
2. Write the full result to `backend/evals/history/` (git-tracked; the golden set itself is not).
...
4. Compare against the most recent file in `evals/history/`. **Any critical field down more than
